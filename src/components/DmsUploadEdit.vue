  ...
  <div>
    <template>
      ...
      <Row v-for="(doc, index) in listDocument" :key="index">
        ...
        {{ doc.attachTopicName }}&nbsp;<span style="color: red" v-if="options.flagRequire">*</span>&nbsp;&nbsp;<small class="color-gray">{{ doc.version == 0 ? '' : '(V.'+doc.version+')' }} </small><small class="color-gray"> {{ doc.version == 0 ? '' : doc.updatedDate | dfDatetimeTH }}</small>
        </Col>
...
export default {
      ...
      listDocument: [
        {
          'pageIndex': 0,
          'pageSize': 0,
          'createdDate': '05/11/2019 18:36:23',
          'createdBy': 'dev',
          'createdByName': null,
          'updatedDate': '05/11/2019 18:36:30',
          'updatedBy': null,
          'updatedByName': null,
          ...
          'sizeLimit': 0,
          'scanFlag': 'Y',
          'chronicle': '313b6ff0-edda-47e1-9ede-d8507a406df0',
          'scanVersion': null,
          'refNo': '108885-000001/2562',
          'callbackRefId': '108885-000001/2562',
          'activeFlag': 'Y',
  ...
  methods: {
    ...
    viewDocs(data) {
      let routeData = this.$router.resolve({ name: 'document-form-view', query: { chronicle: data.chronicle, version: data.version } })
      window.open(routeData.href, '_blank')
    },
    async editDocs (data) {
      this.$alertbox.info('ขออภัย ไม่สามารถอัพโหลดเอกสารได้จริงในระบบนี้')
    },
    modalOnClose() {
  ...
  mounted () {
    this.options.flagName = this.getFlagName
    this.options.flagDownloadForm = this.getFlagDownloadForm
    // console.log('ZZZ', this.getFlagName)
    console.log('#DEMO-UPLOAD-DOCUMENT: mounted()', this.dmsParams.scanDocIds)
    // console.log('#AAA: ', this.scanDocId != null ? this.scanDocId.length : null)
    if (this.dmsParams.scanDocIds != null && this.dmsParams.scanDocIds.length > 0) {
      this.dmsParams.scanDocIds.map((id, index) => { // FROM PARENT
        // console.log('INDEX ', index)
        if (this.scanDocId == null) {
          this.scanDocId = []
        }
        // console.log('this.scanDocId ', this.scanDocId)
        // console.log('KKO')
        // let aa = parseInt(id)
        // console.log('KKO2')

        console.log('#mounted : dmsParams.scanDocIds map ', id)
        this.scanDocId.push(id != null ? parseInt(id) : '')
      })
      // console.log('#############################INIT scanDocId', this.scanDocId)
    }

    // console.log('#BBB: ', this.scanDocId != null ? this.scanDocId.length : null)
    this.getDataDocumentList()
  }
}
</script>
