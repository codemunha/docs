<template>
  <DslCollapse :is-collapse="false">
    <template v-slot:body>
      <VueMarkdown :style="{paddingLeft: '16px'}">{{ md }}</VueMarkdown>
      <!-- <VueMarkdown :style="{paddingLeft: '16px'}" :source="mdText"/> -->
    </template>
  </DslCollapse>
</template>
...
// import MarkDownData from './CHANGELOG.md'
...
- Demo of 5 basics components: auto complete, button, input, modal, and table (custom).`)
  ...
  mounted() {
    ...
    // this.mdText = Log
    // axios.get('src/views/log/1.x/CHANGELOG.md').then(response => { this.mdText = response.data })
  }
}
</script>